...
Let's create a simple `bash` script which will extract this data, and learn how to use this script in Trickest Platform to parse it whenever we want to brute-force different web server.
...
# Use jq to extract relevant fields and format them into a single line per entry
jq -r '.results[] | "URL: \(.url), Status: \(.status), Content-Type: \(.["content-type"]), Length: \(.length), Words: \(.words), Redirect Location: \(.redirectlocation)"' "$input_file" > "$output_file"
...
In the Left Sidebar, we can search for `custom-script` and drag & drop it to the canvas.

<Frame caption="Python Script Node example">
 <img src="/images/custom-script.png"></img>
</Frame>

<Tip>If you want to use the `file` output from bash scripts outputs and connect it to other nodes, that file must be in `out/output.txt` path</Tip>
...
Now, let's get back to our `ffuf` workflow and add custom bash script. We are also going to connect `ffuf` output to the `custom-script` input.

<Frame caption="Connecting Custom Script to `ffuf`">
 <img src="/images/custom-script-ffuf.gif"></img>
</Frame>

Now, let's copy the script we created above and paste it in the `custom-script` node.

<Frame caption="Pasting the script">
 <img src="/images/custom-script-paste-script.gif"></img>
</Frame>
...
<div style={{ display: 'flex', alignItems: 'left' }}>
  ...
  <div style={{ flex: '2' }}>
  <Frame>
    <img src="/images/custom-script-generated-paths.png" alt="Workflow Editor Right Sidebar" style={{ width: '200px', height: '100%' }} />
  </Frame>
  </div>
  </div>
...
input_file = 'ffuf-output.txt' -> input_file = 'in/ffuf-1/output.txt'
